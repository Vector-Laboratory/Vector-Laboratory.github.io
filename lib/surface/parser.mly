...
let rec expr_to_pat (e : expr) : pat =
  ...
  | Var (_, x)            -> PVar (l, x)
  | Ctor (_, c)           -> PCtor (l, c, [])
  | App (_, Ctor (_, c), arg) -> PCtor (l, c, [expr_to_pat arg])
  | Lit (_, lit)          -> PLit (l, lit)
  | Tuple (_, es)         -> PTuple (l, List.map expr_to_pat es)
  | Annot (_, e', t)      -> PTyped (l, expr_to_pat e', t)
  | Bop (_, "|", p1, p2) ->
    (match expr_to_pat p1, expr_to_pat p2 with
     ...
     | p1', p2'                   -> POr (l, [p1'; p2']))
  | App (_, f, arg) ->
    (* Multi-arg constructor: Foo a b → PCtor(Foo, [a;b]) *)
    let rec flatten acc = function
      | App (_, Ctor (_, c), a) -> PCtor (l, c, List.map expr_to_pat (a :: acc))
      | App (_, f', a)          -> flatten (a :: acc) f'
      | _                       -> raise (Failure "not a valid pattern")
    in flatten [arg] f
  | FieldName (_, n)       -> PVar (l, n)   (* .x used as var name in patterns *)
  | _                      -> raise (Failure "not a valid pattern")
...
decl :
  ...
  | DISTINCT n = UPPER tvs = tvar_list LARROW t = ty
    { DDistinct (loc (), n, tvs, t) }

  (* Effect definition *)
  | EFFECT n = UPPER ps = lower_list LPAREN ops = effect_ops RPAREN
    { DEffect (loc (), n, ps, ops) }

  (* Module *)
  | MODULE n = UPPER body = module_body
    { let (iface, impl) = body in DModule (loc (), n, iface, impl) }
  ...
  | EXTERN s = STRING n = LOWER COLON t = ty body = extern_body_opt
    { let _ = s in DExtern (loc (), [], n, t, body) }
  | EXTERN n = LOWER COLON t = ty body = extern_body_opt
    { DExtern (loc (), [], n, t, body) }

  (* Extern — expression-body with parameter list:
       extern "C" f (p : T, …) : R = "c_expr"
       extern f (p : T, …) : R = "c_expr"
     The return type uses fun_ty (not ty/row_ty) so the OP "=" is not
     greedily consumed by the row_ty → row_ty OP fun_ty rule. *)
  | EXTERN s = STRING n = LOWER
    LPAREN ps = extern_params RPAREN COLON t = fun_ty
    o = OP body = STRING
    { if o <> "=" then failwith ("extern body: expected '=' but got '" ^ o ^ "'");
      let _ = s in DExtern (loc (), [], n, t, Some (EBExpr (ps, body))) }
  | EXTERN n = LOWER
    LPAREN ps = extern_params RPAREN COLON t = fun_ty
    o = OP body = STRING
    { if o <> "=" then failwith ("extern body: expected '=' but got '" ^ o ^ "'");
      DExtern (loc (), [], n, t, Some (EBExpr (ps, body))) }

  (* Derived *)
  | DERIVED n = LOWER ann = ann_opt LARROW e = expr
    { DDerived (loc (), n, ann, e) }
  ...
  (* Isomorphism *)
  | ISO tvs = tvar_list_opt p1 = pat COMMA p2 = pat
    LPAREN from1 = from_impl SEMI from2 = from_impl RPAREN
    { DIso (loc (), [], tvs, p1, p2, from1, from2) }

  (* Trait *)
  | TRAIT tvs = tvar_list n = UPPER ps = lower_list body = trait_body
    { DTrait (loc (), tvs, n, ps, body) }

  (* Implementation *)
  | IMPLEMENTATION tvs = tvar_list n = UPPER ts = nonempty_ty_list body = impl_body
    { DImpl (loc (), tvs, n, ts, body) }

  (* Hook definitions — must come before the catch-all DVal rule *)
  | tvs = tvar_list_opt kw = hook_kw hh = hook_head LARROW e = expr
    { DHook (loc (), [], tvs, kw, hh, e) }
  ...
  | p = val_lhs LARROW e = expr
    { DVal (loc (), [], to_pat "value binding" p, None, e) }
...
hook_head :
  ...
  | sym = hook_sym ep = effect_param_opt ps = pat_csv
    { HHSym (sym, ep, ps) }
  ...
  (* Positional: project / assign / view / from  [e] T, R *)
  | ep = effect_param ps = pat_csv
    { HHPos (Some ep, ps) }
...
module_body :
  ...
  | LPAREN ds = decl_list RPAREN
    { (None, Some ds) }
  (* Layout form: the layout preprocessor already handles the block body,
     emitting VSemi between decls.  The block is delimited by closing
     context.  We use the paren form exclusively for now. *)
...
trait_sig :
  | tvs = tvar_list_opt kw = hook_kw hh = hook_head COLON t = ty def = default_opt
    { { ts_attrs = []; ts_kw = kw; ts_head = hh; ts_result = t; ts_default = def } }
...
open_filter_opt :
  ...
  | LOWER LPAREN items = import_item_list RPAREN
    (* "hiding" keyword *)
    { Some (OFHiding items) }
...
atom :
  | i = INT      { Lit (loc (), classify_lit i) }
  | f = FLOAT    { Lit (loc (), LFloat f) }
  | s = STRING   { Lit (loc (), LStr s) }
  | s = SYMBOL   { Lit (loc (), LSymbol s) }
  | n = LOWER    { match n with
                   | "true"  -> Lit (loc (), LBool true)
                   | "false" -> Lit (loc (), LBool false)
                   | _       -> Var (loc (), n) }
  | n = UPPER    { if n = "Never" then Lit (loc (), LUnit)   (* placeholder; ⊥ is a type *)
                   else Ctor (loc (), n) }
  | NEVER        { Ctor (loc (), "Never") }
  | t = TICK     { TickVal (loc (), t) }
  (* Field name literal: .x *)
  | DOT n = LOWER   { FieldName (loc (), n) }
  (* Parenthesised / tuple / branch / handler block *)
  | e = paren_expr  { e }
  (* Array literal *)
  | LBRACKET es = array_elems RBRACKET  { Array (loc (), es) }
  (* Record literal *)
  | LBRACE fs = record_field_list RBRACE  { Record (loc (), fs) }
  (* Quoted program `[expr] *)
  | BACKTICK_LBRACKET e = expr RBRACKET  { Quoted (loc (), e) }
  (* Unquoted splice .(expr) *)
  | DOT LPAREN e = expr RPAREN           { Splice (loc (), e) }
...
expr :
  ...
  | base = expr t = TICK rhs = tick_rhs
    { let (opt_v, opt_a) = rhs in make_tick_app (loc ()) base t opt_v opt_a }
...
tick_rhs :
  ...
  (* Two-arg form: value? = first atom, proj_arg = second *)
  | v = atom a = proj_arg        { (Some v, Some a) }


(** [proj_arg] — the argument slot in a tick or operator application.
    Does NOT include unparenthesised lambdas (must be in parens). *)
proj_arg :
  | n = LOWER                              { Var (loc (), n) }
  | n = UPPER                              { Ctor (loc (), n) }
  | o = OP                                 { Section (loc (), SBareOp o) }
  | LPAREN e = expr RPAREN                 { e }
  (* Handler block as proj_arg for 'handle: arr 'handle (ctl …) *)
  | LPAREN cs = handler_clauses RPAREN     { Section (loc (), SHandle cs) }
...
tuple_expr :
  | e = expr                              { e }
  | e = expr COMMA rest = tuple_rest      { Tuple (loc (), e :: rest) }
...
pat :
  | p = pat_or                              { p }
  | p = pat_or AT n = LOWER                { PAs (loc (), p, n) }
  (* Typed pattern *)
  | LPAREN p = pat COLON t = ty RPAREN     { PTyped (loc (), p, t) }
...
pat_app :
  | p = pat_atom                            { p }
  (* Constructor with one or more args *)
  | c = UPPER ps = pat_app_args             { PCtor (loc (), c, ps) }

pat_app_args :
  | p = pat_atom                            { [p] }
  | ps = pat_app_args p = pat_atom          { ps @ [p] }

(** Atomic pattern — no further application *)
pat_atom :
  | n = LOWER   { match n with
                  | "_"     -> PWild
                  | "true"  -> PLit (loc (), LBool true)
                  | "false" -> PLit (loc (), LBool false)
                  | _       -> PVar (loc (), n) }
  | n = UPPER   { PCtor (loc (), n, []) }
  | NEVER   { PLit (loc (), LUnit) }   (* ⊥ as literal placeholder *)
  | i = INT     { PLit (loc (), classify_lit i) }
  | f = FLOAT   { PLit (loc (), LFloat f) }
  | s = STRING  { PLit (loc (), LStr s) }
  | s = SYMBOL  { PLit (loc (), LSymbol s) }
  (* Tuple / or-pattern / typed pattern in parens *)
  | LPAREN RPAREN           { PLit (loc (), LUnit) }
...
ty_atom :
  ...
  (* Pointer type *)
  | LOWER (* "Ptr" *) t = ty_atom
    { if $1 = "Ptr" then TPtr t
      else failwith ("unexpected type application of " ^ $1) }
